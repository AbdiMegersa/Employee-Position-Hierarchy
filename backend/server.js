...
app.get('/api/array-roles', (req, res) => {
    res.status(400).json(arrayRolesData)
})

app.get('/api/employees', (req, res) => {
  console.log(parsedEmployeeData)
  res.status(200).json(parsedEmployeeData)
})

// get all nested roles
app.get('/api/nested-roles', (req, res) => {
    res.status(400).json(parsedRolesData)
})

// get roles by id
app.get('/api/role/:id', (req, res) => {
    const roleId = req.params.id;
    const role = arrayRolesData.find(role => role.id === parseInt(roleId));
...
app.post('/api/new-role', async (req, res) => {
  ...
  // check if roleExist
  const role = arrayRolesData.find(_role => _role.id === newRole.id)
  
  if(!role){
    const {parentId, ...roleData} = newRole   
    const parentExist = arrayRolesData.find(_role => _role.id === parentId)
    if(!parentExist){
      res.status(404).json({message: 'Parent Role not found'})
      return
    } 
    parsedRolesData = addNodeToNestedStructure(parsedRolesData, roleData, parentId)
    ...
    res.status(200).json(parsedAllData);

  }else{
    res.status(403).json({message: 'Role with the same id already exists'})
  }
...
app.post('/api/new-employee', (req, res) => {
  ...
  if(employeeExist){
    ...
    const roleExist = arrayRolesData.find(r => r.id===roleId)
    if(roleExist){
      ...
      res.status(400).json({message: `Role with id${roleId} not found!`})
    }
  }
  });
...
app.put('/api/update-role/:id', (req, res) => {
    ...
    let role = arrayRolesData.find(role => role.id === roleId);
    ...
    if(role) {
        ...
        if(newRole.data.parentId === role.data.parentId){
          ...
          parsedRolesData = replaceNodeById(parsedRolesData, role.id, newRole);
        }else{
        ...
        arrayRolesData = convertNodeToArray(parsedRolesData);
        writeFileSync(filePath, JSON.stringify(parsedAllData))
        res.status(200).json(parsedRolesData);
    }else{
...
app.put('/api/update-employee/:id', (req, res) => {
    ...
    if(employee){
      ...
      if(employee.roleId === employeeDetail.roleId){
        ...
        const newRoleExist = arrayRolesData.find(role => role.id === employeeDetail.roleId)
        if(!newRoleExist ){
          res.status(404).json({message: `role with id ${employeeDetail.roleId} not found`})
        }else{
          console.log('CHANGE')
...
app.delete('/api/delete-role/:id', async (req, res) => {
  const roleId = parseInt(req.params.id);
  const roleExist = arrayRolesData.find((role) => role.id === roleId);
  ...
  if (roleExist) {
    ...
    if (roleExist.children && !roleExist.children.length > 0) {
      ...
      res.status(403).json({ message: 'Cannot delete role that has children', role: roleExist });
    }
  } else {
    res.status(404).json({ message: 'Role not found' });
  }
...
app.delete('/api/delete-employee/:id', async (req, res) => {
  ...
  if(employee){
    const role = arrayRolesData.find(role => role.id === employee.roleId)
    if(role){
